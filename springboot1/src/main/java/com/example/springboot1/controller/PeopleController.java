...
public class PeopleController {             //实现前后台数据交互
    ...
    {
        People res=peopleMapper.selectOne(Wrappers.<People>lambdaQuery().eq(People::getUsername,people.getUsername()));
        if (res !=null){
...
//未知
    ...
    public Result<?> getById(@PathVariable Long id) {
        return Result.success(peopleMapper.selectById(id));
    }
    ...
    public Result<?> findPage(@RequestParam(defaultValue = "1") Integer pageNum,/**当前页,数据页面当前翻到的页码*/
        ...
        LambdaQueryWrapper<People> wrapper = Wrappers.<People>lambdaQuery().orderByDesc(People::getNickName); //SQL server 排序
